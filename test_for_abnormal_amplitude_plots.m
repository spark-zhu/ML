...
% data=data';
for seg=1:5
    clearvars -except seg data dataFile
clc
currentpath=pwd;
...
cluster=h5read(KwikName,'/channel_groups/0/spikes/clusters/main');
MyTimes=h5read(KwikName,'/channel_groups/0/spikes/time_samples');
...
WCList = zeros(numel(list),2);

for clu=15:15%numel(list)
    clu
...
% above100 = find(ValleyAcrossTime>-100);
% above100 = above100(seg);
% unit = unit(:,:,above100);
% FiringTimeForThisUnit = FiringTimeForThisUnit(above100);
% clear ISI_in_MS_bins counts
% ISI_in_MS_bins = diff(double(FiringTimeForThisUnit))/20; % Intervals in miliseconds. 
% [counts] = histc(ISI_in_MS_bins,0:1:50);
% ValleyAcrossTime=ValleyAcrossTime(above100);
...
%         set(ax,'FontWeight','bold')
        ...
        if Ch_Map(row,col)~=0
            ...
            if min(min(MeanPlot))<-100 || max(max(MeanPlot))>25
                plot(t,MeanPlot(temp,:),'Color',color(floor(Intensity(temp)*100)+1,:),'LineWidth',2)
%                 axis([t(1) t(end) -150 50])
                axis([t(1) t(end) min(min(MeanPlot)) max(max(MeanPlot))])

            else
                plot(t,MeanPlot(temp,:),'Color',color(floor(Intensity(temp)*100)+1,:),'LineWidth',1)
...
set(ax,'Visible','off')
 ...
 ax=axes('Units','centimeters','position', [0.6 0.6 5 5]);
        set(ax,'FontSize',6)
        if ~isempty(counts)
bar(0.5:1:50.5,counts)
        end

        ax=axes('Units','centimeters','position', [6 0.6 8 5]);
        set(ax,'FontSize',6)
