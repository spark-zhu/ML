...
cd('D:\Xue0109_tracking\surgery 2017-01-09')
% clear all 
% close all
% warning off
load('syncFolder')
...
% distributionOfLabor = 1:50;% this instance of matlab only in charge of the first 8 files. 

%% Estimate Noise for each day and report suggested cutOffs for noise removal plots. 

NoiseCollection = cell(1,numel(distributionOfLabor));
for folder = distributionOfLabor
folderName = folderList(folder).name
storageFolder = [syncFolder '\' folderName];
cd(folderName)% cd(storageFolder)
 load('stdV')
 NoiseCollection{folder} = stdNoise;
cd .. 
end
%%
for folder = distributionOfLabor
