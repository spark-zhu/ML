function read_and_replot_Intan_file
...
[file, path, filterindex] = ...
    uigetfile('*.rhd', 'Sele  ct an RHD2000 Data File', 'MultiSelect', 'off');
...
for i=1:ch_num
  plot(t_amplifier, amplifier_data(i,:)+(i-1)*offset,'-','color',co(mod(i,7)+1,:)); hold on; 
...
title(sprintf(file));
   ...
   datafilt = zeros(size(amplifier_data));
   figure(2); clf; co=get(gca,'colororder'); 
   offset = 200;
   for i=1:ch_num
       datafilt(i,:)  = filter(Hd,amplifier_data(i,:));
       plot(t_amplifier, datafilt(i,:)+(i-1)*offset,'-','color',co(mod(i,7)+1,:)); hold on; 
       text(max(t_amplifier)+2,(i-1)*offset,sprintf(amplifier_channels(i).native_channel_name),'color',co(mod(i,7)+1,:));
   end
 title([sprintf(file) 'filtered'])   
return
