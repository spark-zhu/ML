...
Source = memmapfile([dataFile '.mda'],'Offset',20, 'Format', {'int16', [nChansInRawFile, numOfCol],'x'},'Writable',true);
...
%%
   ...
   if exist('Dat_V_Map.mat', 'file')
        load('Dat_V_Map.mat')
   
    elseif exist([mainFolderPath '\Dat_V_Map.mat'], 'file')
...
clear unit %MeanPlot
for k=1:length(p)
    unit(:,:,k)=1/100*double(Source.Data.x(1:nChansInDatFile,MyTimes(p(k))-tBefore:MyTimes(p(k))+tAfter));
...
PeakAcrossTime = max(unit(ch,:,:),[],2);  
ValleyAcrossTime = unit(ch,11,:);
...
T=text(.1,.3,[num2str(sum(cluster==list(clu))/(numOfCol/20E3)) '  spk/sec']); %
...
%         result.Intensity{clu}=StandardIntensity;
        ...
        result.PeakAcrossTime=result.PeakAcrossTime(newList>0);
...
%         result.Intensity=result.Intensity(I);
        ...
        result.PeakAcrossTime=result.PeakAcrossTime(I)';
...
%  hold on
%  load('Locations')
%  for unitNumber = 1:size(WCList,1)
%  
%  if newList(unitNumber)==1
%     
%  scatter3(WCList(unitNumber,1),WCList(unitNumber,2),abs(WCList(unitNumber,3)),(P2PList(unitNumber)/10).^2,'filled','MarkerFaceColor','green') 
%  else
%      scatter3(WCList(unitNumber,1),WCList(unitNumber,2),abs(WCList(unitNumber,3)),(P2PList(unitNumber)/10).^2,'filled','MarkerFaceColor','red') 
%  end
%  
%  end
% view(30, 60);
