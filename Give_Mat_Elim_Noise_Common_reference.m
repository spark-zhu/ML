...
% 
% MedianNoise = median(amplifier_data);
% tureNoise = zeros(size(MedianNoise));
% for ele = 1:numel(tureNoise)
% tureNoise(ele)=mean(amplifier_data( amplifier_data(:,ele)>MedianNoise(ele),ele));
% end
% 
% amplifier_data = amplifier_data - repmat(tureNoise,size(amplifier_data,1),1);
% % 
